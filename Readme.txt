...
48. 조회 오류
    Supabase에서 가져온 Review 데이터는 단순한 글자가 아니라, 
    Supabase 라이브러리의 복잡한 기능들(PrimaryKeyAttribute 등)이 
    덕지덕지 붙어있는 무거운 객체입니다.
    "이 PrimaryKey라는 스티커는 텍스트로 못 바꾸겠는데?" 하고 에러를 뱉은 것
    이전에는 우리가 Select로 예쁘게 포장해서 보냈는데, 
    필터링 로직을 추가하면서 포장하는 과정을 빼먹어서 그렇습니다!

    ✅ 해결 방법: "데이터 포장하기" 부활
        **백엔드(ReviewController.cs)**의 GetReviews 함수에서, filteredReviews를 그대로 내보내지 말고, 필요한 알맹이만 골라서(Select) 보내주면 해결됩니다.

49. 로그아웃 버튼 하단으로 보내고, 내 서재와 모두의 서재 나누기
    백엔드는 건드릴 필요가 없습니다! 
    (아까 만든 백엔드 로직이 이미 "공개된 모든 글 + 내 글"을 다 가져오고 있기 때문에, 프론트엔드에서 **분류(Filter)**만 잘해주면 됩니다.)

    🛠️ 주요 변경 사항
        탭 3개로 분리: 🔍검색 / 🌏모두의 서재 / 📚내 서재

        데이터 필터링:

        모두의 서재: isPublic이 true인 것만 보여줌.

        내 서재: userId가 나(session.user.id)랑 똑같은 것만 보여줌.

        UI 변경: 로그아웃 버튼을 **화면 맨 아래(Footer)**로 이동.

50. 모두의 서재는 read only, 나의 서재만 editable
    🛠️ 수정할 포인트
        상태 추가: isReadOnly (읽기 전용 모드인지 확인하는 변수)

        클릭 로직 변경:

        모두의 서재에서 클릭 → 무조건 읽기 전용으로 열기.

        내 서재에서 클릭 → 수정 모드로 열기.

        모달 화면 변경:

        읽기 전용일 때는 에디터(입력창) 대신 그냥 글 내용을 보여줌.

        읽기 전용일 때는 [수정], [삭제] 버튼 숨김.
...
60. 리뷰 쓰는 모달창에도 댓글이 보이는 문제 수정
    우리가 모달(Modal) 창 하나를 가지고 "글 쓰기(Write)", "수정하기(Edit)", "읽기(Read)" 3가지 용도로 돌려쓰고 있잖아?

    그런데 댓글 섹션(div)을 그냥 놔두면, 컴퓨터는 눈치가 없어서 "글 쓸 때도 댓글 창을 보여줘야 하나 보다" 하고 무작정 보여주는 거야. (심지어 새 글이라 댓글이 없으면 "첫 댓글을 남겨보세요"라는 문구라도 띄울 거야.)

    "읽기 모드(Read Only)일 때만 댓글 보여줘!" 라고 조건을 걸어주면 깔끔하게 해결돼.
